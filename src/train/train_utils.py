...
def cos_sim_mat_generate(emb1, emb2, device, bs=128):
    ...
    array_emb2 = F.normalize(torch.FloatTensor(emb2), p=2, dim=1)  # [10500, 300]
    res_mat = batch_mat_mm(array_emb1, array_emb2.t(), device, bs=bs)
    return res_mat  # torch.Size([10500, 10500])
...
def batch_mat_mm(mat1, mat2, device, bs=128):
    ...
    for i in range(0, axis_0, bs):
        ...
        res = temp_div_mat_1.mm(mat2)  # [bs, emb], [Ent, emb] -> [bs, Ent]
...
def batch_topk(mat, bs=128, topn=50, largest=False):
    ...
    axis_0 = mat.shape[0]
    for i in range(0, axis_0, bs):
        temp_div_mat = mat[i:min(i + bs, axis_0)]  # torch.Size([128, 10500])
        score_mat, index_mat = temp_div_mat.topk(topn, largest=largest)  # topk score, and index
        res_score.append(score_mat.cpu())  # [128, 128]
        res_index.append(index_mat.cpu())
    res_score = torch.cat(res_score, 0)  # [10500, 128]
    res_index = torch.cat(res_index, 0)  # [10500, 128]
...
def hits(index_mat):
    ...
    # index_mat[i][i] == 1 means predict right
    ...
    # print(mrr_mat)
    # mr_mat = [sum([(j + 1) if index_mat[i][j] == i else 0 for j in range(cands_num)]) for i in range(ent1_num)]
    # print(mr_mat)
    ...
    print('sum(mrr_mat)', sum(mrr_mat))
    print('len(mrr_mat)', len(mrr_mat))
    # mr_value = sum(mr_mat) / len(mr_mat)
    # print('sum(mr_mat)', sum(mr_mat))
    # print('len(mr_mat)', len(mr_mat))
    result_title_str += 'MRR'
    result_str += str(mrr_value)
    # result_title_str += '\tMR'
    # result_str = result_str + '\t' + str(mr_value)
