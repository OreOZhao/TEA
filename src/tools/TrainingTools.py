...
class TrainingTools:
    ...
    def batches(self, get_bc, type=''):
        self.__init_metrics()
        with tqdm(total=self.sample_count, file=sys.stdout, ncols=80) as pbar:
            done = 0
            for i, batch in enumerate(self.iter):
                ...
                pbar.set_description(type + ' Loss: %f\tAcc: %f\tPrec: %.2f\tRec: %.2f\tF1: %.2f' % (
                    self.loss_count / done, self.accuracy, self.precision, self.recall, self.f1_score))
    ...
    def batch_iter(iter: DataLoader, desc=''):
        ...
        with tqdm(total=total, file=sys.stdout, ncols=80) as pbar:
            done = 0
