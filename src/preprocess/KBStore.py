...
class KBStore:
    ...
    def save_seq_form(self, dicts: Iterator, header: List):  # here save sequence for entity
        ...
        def get_neighboronly_seq(dic: dict):
            eid = dic['ent_id']
            values = []
            ename = dic['ent_name']
            if ename in self.text_facts:
                self.text_facts[ename].sort(key=lambda x: (x[0], x[1]), reverse=False)
                for f in self.text_facts[ename]:
                    seq = f[1]
                    values.append(seq)
            seq = ename + "'s neighbors are " + ', '.join(values)
            assert len(seq) > 0
            return eid, seq

        seq_path = self.dataset.neighboronly_seq_out
        print(Announce.doing(), 'Save', seq_path)
        seqs = MPTool.packed_solver(get_neighboronly_seq).send_packs(dicts).receive_results()
        ...
        if file_type == OEAFileType.attr:
            ...
            for tok in toks:
                ...
                if bool(re.search(r'\d', tok)):
                    return
            sbj_id = self.get_or_add_item(sbj, self.entities, self.entity_ids)  # entity
            ...
            self.add_fact(obj, pred + '-', sbj, self.text_facts)
